...
from spellcardoverlay import draw_card_overlay  # Front overlay
...
def generate_background(spell_name, element, effect_lore):
    ...
    prompt = (
        f"Art Nouveau-style illustration of a magical spell called'{spell_name}' "
        f" "
        f"."
    )
...
def generate_final_pdf(front_images, back_images, filename, card_width=750, card_height=1050, dpi=300):
    ...
    for i in range(0, len(front_images), cards_per_page):
        ...
        for j in range(len(back_batch)):
            ...
            x = int(x_start + col * (card_width_pts + 24))  + 2# **Apply same right shift**
            y = int(y_start - (row + 1) * (card_height_pts + 24))
...
for index, row in df.iterrows():
    spell_name = row.get('Spell Name', 'Unknown Spell')
    element = row.get('Element', 'Unknown Element')
    effect_lore = row.get('Effect/Combo', 'No Effect')
    attack = str(row.get('Attack', 0))  # Ensure it's a string for display
    modifier = row.get('Modifier', 'Unknown Modifier')
    modifier_desc = row.get('modifer description', 'No Description')
